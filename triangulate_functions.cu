...
template< typename T >
__device__ float area( T* contour, int cell_index, int size, int i_, int j_ )
{
    int n = size;
//  if ( (i_ == 1) && (j_ == 1) )
//  {
...
template< typename T >
__device__ bool snip( T* contour, int cell_index, int u, int v, int w, int n, int* V )
{
    int p;
...
template< typename T >
__device__ bool process( T* contour, T* result, int cell_index, int size, int i_, int j_ )
{
...
//  }
    ...
    if( 0.0f < area< T >( contour, cell_index, size, i_, j_ ) )
    {
        for( int v = 0; v < n; v++ )
        {
            V[ v ] = v;
    ...
    for( int m = 0, v = nv - 1; nv > 2; )
    {
        ...
        if( snip< T >( contour, cell_index, u, v, w, nv, V ) )
        {
            int a, b, c, s, t;
...
template< typename T >
__device__ void triangulate_polygon( T* diagram, int* edge_count, int cell_index, int cell_size, int i_, int j_ )
{
    T result[ CELL_SIZE ];
...
//  Invoke the triangulator to triangulate this polygon.
    process< T >( diagram, result, cell_index, edge_count[ cell_index ], i_, j_ );
...
//  }
    ...
    /* Retrieve result and sum pixel offset */
    for( int i = 0; i < cell_size; i++ )
    {
        diagram[ i ].x = result[ i ].x + ( ( float )i_ );
        diagram[ i ].y = result[ i ].y + ( ( float )j_ );
//      if ( (i_ == 1) && (j_ == 1) )
        //printf(" triangulate  P( %2.2f, %2.2f )\n", result[i].x, result[i].y);
    }
    for( int i = cell_size; i < CELL_SIZE; i++ )
    {
        diagram[ i ].x = 0.0;
        diagram[ i ].y = 0.0;
//      if ( (i_ == 1) && (j_ == 1) )
